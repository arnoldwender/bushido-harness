#!/usr/bin/env python3
"""The Bushido Harness gate: every attributed quotation must trace to a source.
...
which is the exact failure 誠 Makoto never trades. This gate
...
A repo whose third virtue is 誠 Makoto — "invent nothing: no fabricated
number, citation, source, path, or command" — cannot ship a citation nobody
can test.
"""
...
def check_urls_online(sources: list[Source], findings: list[Finding]) -> None:
    ...
    for s in sources:
        ...
        req = urllib.request.Request(url, method="HEAD", headers={
            "User-Agent": "bushido-harness-citation-gate/1.0"})
        try:
...
def to_sarif(findings: list[Finding]) -> dict[str, Any]:
    return {
        ...
        "runs": [{
            "tool": {"driver": {
                "name": "bushido-harness-citations",
